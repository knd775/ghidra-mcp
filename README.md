...
- **268 MCP tools** — 3x more than any competing implementation. Not just read operations — full write access for renaming, typing, commenting, structure creation, script execution, P-code emulation, and live debugging.
...
- **268 MCP tools** — Comprehensive API surface covering every aspect of binary analysis
...
| `GHIDRA_MCP_BSIM_URLS` | Comma-separated allowlist of BSim network URLs (`postgresql://host[:port]/database`). Fail-closed: unset rejects every network `db_url`. Compose lists `bsim` on both compose DNS and `BIND_ADDR`. |
...
- `builder_health` - What the ghidra-builder container can compile right now: packed toolchain identities, ARM GNU releases, and framework stubs
- `corroborate_match` - Compare listing-level constants, strings and direct callees of an open query function against a BSim corpus candidate
- `source_read` - Read a span of pinned reference source from the builder cache
