...
- **Reference builder**: `docker/Dockerfile.builder`, image `ghidra-builder` (one service, gcc10-arm / gcc12-arm / gcc13-arm as pinned ARM GNU prefixes, gcc13-x86_64 as distro gcc-13). Identity is `<compiler><major>-<target>`, not the image tag. Pins: `docker/builder/toolchains.lock`. Always-on, uid 1000, no host ports, no docker.sock. `POST /build` returns a job id; `GET /build/{id}` is the poll. Corpus definition: `docker/references.yaml` (ARM) and `docker/references.userland.yaml` (x86-64, `medium_64`). Framework stubs: `docker/stubs/<framework>/` (shipped: `pico-sdk`, musl, glibc, openssl, libsodium, sqlite). `mode=framework` harvests per-library `.o`/`.a` from the build tree, never the linked ELF. Compile keeps DWARF (`/ref/<name>/` prefix). `source_read` reads the builder source cache.
