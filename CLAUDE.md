...
MCP server bridging Ghidra reverse engineering with AI tools. 263 MCP tools for binary analysis.
...
- **Service Layer**: `src/main/java/com/xebyte/core/` -- 16 service classes (~20K lines), `@McpTool`/`@Param` annotated. v5.4.0 adds `EmulationService` (P-code emulation), `DebuggerService` (TraceRmi wrapping — GUI-only). v7.0.0 adds `BSimService` (CLI wrap of `support/bsim`; BSim is **not** on the server classpath) and `ReferenceBuildService` (`build_reference` / `build_manifest` — sibling `ghidra-builder:gccN` containers compile pinned source onto the shared `/data` volume).
- **Reference builder**: `docker/Dockerfile.builder`, tagged `ghidra-builder:gcc10` / `:gcc12` / `:gcc13` (never `:latest`). Compose service, uid 1000, no host ports. Corpus definition: `docker/references.yaml`.
...
- **Authoritative repo snapshot**: `tests/endpoints.json` (263 endpoints, categories, descriptions)
...
| `src/main/java/com/xebyte/core/ReferenceBuildService.java` (`ReferenceBuild`, `ReferenceManifest`, `BuilderClient`) | Offline (Java) `ReferenceBuildServiceValidationTest` + `tests/unit/test_builder_stack.py`. `dry_run=true` must not call the builder. Manifest `docker/references.yaml` expands to nine littlefs jobs. Do not mount docker.sock into ghidra-mcp. |
