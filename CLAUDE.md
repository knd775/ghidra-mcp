...
MCP server bridging Ghidra reverse engineering with AI tools. 265 MCP tools for binary analysis.
...
| `src/main/java/com/xebyte/core/ReferenceBuildService.java` (`ReferenceBuild`, `ReferenceManifest`, `FrameworkBuild`, `BuilderClient`, `ToolchainIdentity`) | Offline (Java) `ReferenceBuildServiceValidationTest` + `tests/unit/test_builder_stack.py`. `dry_run=true` must not `POST /build` (no clone, no compile). It does `GET /health`; that list is what unknown names quote. Manifest `docker/references.yaml` expands to nine littlefs jobs plus twelve pico-sdk framework jobs (`gcc10-arm`/`gcc12-arm`/`gcc13-arm` × opt × board). `mode=framework` harvests build-tree objects, never the linked ELF. Each artifact gets a `<artifact>.json` sidecar (resolved commit, compiler `--version`, sha256); manifest skip is sidecar-hash match, not filename. One `builder` service; identity selects binaries inside that image. `POST /build` returns a job id; `wait_seconds` (max 55) then `build_reference_status`. `builder_health` proxies `GET /health`. Do not mount docker.sock into any service. |
