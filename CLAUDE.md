...
- **Service Layer**: `src/main/java/com/xebyte/core/` -- 16 service classes (~20K lines), `@McpTool`/`@Param` annotated. v5.4.0 adds `EmulationService` (P-code emulation), `DebuggerService` (TraceRmi wrapping — GUI-only). v7.0.0 adds `BSimService` (CLI wrap of `support/bsim`; BSim is **not** on the server classpath) and `ReferenceBuildService` (`build_reference` / `build_manifest` / `build_reference_status` / `builder_health` — one resident `ghidra-builder` compiles pinned source onto the shared `/data` volume; `builder_health` is the container's packed identities and stubs).
- **Reference builder**: `docker/Dockerfile.builder`, GHCR image `ghidra-mcp-builder` (compose DNS `ghidra-builder`; gcc10-arm / gcc12-arm / gcc13-arm as pinned ARM GNU prefixes, gcc13-x86_64 as distro gcc-13). Identity is `<compiler><major>-<target>`, not the image tag. Pins: `docker/builder/toolchains.lock`. Always-on, uid 1000, no host ports, no docker.sock. Published by `.github/workflows/ghcr.yml` with the other images. `POST /build` returns a job id; `GET /build/{id}` is the poll. Corpus definition: `docker/references.yaml` (ARM) and `docker/references.userland.yaml` (x86-64); both ingest into `postgresql://ghidra-bsim:5432/bsim` (`medium_nosize`). Framework stubs: `docker/stubs/<framework>/` (shipped: `pico-sdk`, musl, glibc, openssl, libsodium, sqlite). `mode=framework` harvests per-library `.o`/`.a` from the build tree, never the linked ELF. Compile keeps DWARF (`/ref/<name>/` prefix). `source_read` reads the builder source cache.
- **BSim PostgreSQL**: `docker/Dockerfile.bsim`, GHCR image `ghidra-mcp-bsim` (compose DNS `ghidra-bsim`). Stock `postgres:16` plus Ghidra's `lshvector` C extension (sources fetched from Ghidra 12.1.2 / `docker/bsim/lshvector.lock`; do not use `support/bsim_ctl`). SSL mandatory (`hostnossl reject`). One database: `bsim` (`medium_nosize`). ARM firmware and x86-64 userland share it so cross-arch matches are queryable; `bsim_query(arch=...)` constrains when you want same-arch only. Ingest refuses a pointer-size mismatch only against sized templates (`medium_32` / `medium_64`). Published on `BIND_ADDR:5432` (VPN/LAN); not on the Cloudflare tunnel. `GHIDRA_MCP_BSIM_URLS` is a fail-closed allowlist; `file:` URLs still use `GHIDRA_MCP_BSIM_ROOT`. Credentials `GHIDRA_MCP_BSIM_USER`/`PASSWORD` are not the Ghidra Server login. `bsim-backup` runs `pg_dump`. Migration: `docker/bsim/MIGRATION.md`. Confirm live `file:/srv/ghidra/bsim/re` is 32-bit-only before migrating it; if polluted, recreate as `medium_nosize`.
...
| `src/main/java/com/xebyte/core/BSimService.java` (`BSimCli`, `BSimJobs`, `BSimUrls`, `BSimDbProbe`, `BSimMatches`, `BSimCliParser`, `CorroborationExtract`, `CorroborationEvidence`, `CorroborationStore`) | Offline (Java) `BSimCliParserTest` + `BSimServiceValidationTest` + `CorroborationTest`. Python: `tests/unit/test_bsim_postgres_stack.py` (image, compose network, SSL, allowlist, lock pin). Live: `tests/integration/test_bsim_cross_build.py` (skips without `GHIDRA_BSIM_FIXTURE`). `bsim_query` defaults `similarity_threshold=0.0` / `confidence_threshold=10.0` — filter on confidence; cross-compiler matches sit at 0.2–0.4 similarity. Whole-program uses sentinel `ALL` (never `-`). Optional `arch`/`executable`/`compiler`/`exclude_md5` are server-side `BSimFilter` atoms. Functions below `min_feature_count` (default 8) return `identifiable=false`; apply skips them unless `apply_unidentifiable`. Default template is `medium_nosize`. `bsim_apply_matches` still requires an explicit `min_confidence`. Do **not** add BSim modules to the server classpath; query runs `BSim_McpQuery.java` in a helper `analyzeHeadless` JVM. Keep `ghidra_scripts/BSim_McpQuery.java` byte-identical to `src/main/resources/bsim/BSim_McpQuery.java`. **Every BSim tool must answer inside its `wait_seconds` (≤55) or return a job ticket** — the CLI-heavy body runs on `BSimJobs`' single worker and `bsim_job_status` serves the result; a handler that blocks past ~60s is re-creating the upstream-gateway `-32603` this design closed (measured live 2026-08-29). Cheap validation (bad `db_url`/`source`, missing credential, no program) must stay synchronous so invalid input is an immediate, specific error. Network `postgresql://` URLs must match `GHIDRA_MCP_BSIM_URLS` (fail-closed if unset) and must not embed a password; set `GHIDRA_MCP_BSIM_PASSWORD` (not the Ghidra Server login). For `ghidra://` sources the child JVM is STOCK Ghidra: env-var credentials do nothing there — pass `--user` and feed the password on stdin, and always close the child's stdin (an open pipe blocks `HeadlessClientAuthenticator`'s prompt for the full 30-min timeout). **Dual ingest (`ghidra://` + `postgresql://`) stdin order is BSim database FIRST, then Ghidra Server** — `generatesigs --bsim ... --commit` runs `BulkSignatures.signatureRepo`, which pulls the vector config out of the DB before `SignatureRepository.process` touches the repository. The other order fed the Ghidra Server password to PostgreSQL and took out the entire PostgreSQL ingest path with `Password for bsim:ERROR Could not authenticate with database` (measured live 2026-08-31); `bsim_create_db`/`bsim_list_corpus` kept working because a DB-only command has one prompt, and H2 kept working because a `file:` DB never prompts — so it presented as "PostgreSQL ingest is broken" rather than "two secrets, one pipe". `BSimServiceValidationTest` pins the order in both directions. Same family in the USERNAME dimension: `BulkSignatures` applies `--user` to the BSim database too whenever the BSim URL has no userinfo, so with `GHIDRA_MCP_BSIM_USER` unset a `ghidra://` ingest would log into PostgreSQL as the Ghidra Server user — `BSimUrls.ambiguousIngestUser` refuses that combination synchronously (verified live: with `postgresql://bsim@...` the CLI logs `BSim DB server info specifies user 'bsim'. Ignoring user name option`, so the injected userinfo is the only thing separating them). The corroboration extract for a `ghidra://` source runs its OWN `analyzeHeadless` and needs **`-p`**: without it `allowPasswordPrompt=false` and `HeadlessClientAuthenticator` returns BADPASSWORD without ever reading the pipe, so `-connect` alone can never authenticate. Query's helper JVM reads `GHIDRA_MCP_BSIM_PASSWORD` in `BSim_McpQuery`. **`bsim_list_databases` must never report configuration as state** — `GHIDRA_MCP_BSIM_URLS` is an allowlist, not an inventory, and the template comes from a sidecar or `GHIDRA_MCP_BSIM_TEMPLATES`, never from the database. Rows carry `configured` (allowlisted) separately from `present` (`BSimDbProbe`'s read-only JDBC probe) and `config_template_source`. `present` is OMITTED when unknown, because the response serializer drops nulls — `probe` is the field that always answers, so a missing `present` must not be read as `false`. This is the tool an operator reaches for when something is already broken: it previously listed `embedded` and `userland` as existing `medium_nosize` databases when neither had ever been created. Do **not** use `support/bsim_ctl`; the image is `ghidra-mcp-bsim`. **Never verify a BSim write path with `dry_run=true`** — the scanner short-circuits before the handler, so dry-run passing proves nothing about the real path. |
| `src/main/java/com/xebyte/core/ReferenceBuildService.java` (`ReferenceBuild`, `ReferenceManifest`, `FrameworkBuild`, `BuilderClient`, `ToolchainIdentity`) | Offline (Java) `ReferenceBuildServiceValidationTest` + `tests/unit/test_builder_stack.py`. `dry_run=true` must not `POST /build` (no clone, no compile). It does `GET /health`; that list is what unknown names quote. Manifest `docker/references.yaml` expands to eighteen littlefs jobs, twelve pico-sdk framework jobs (`gcc10-arm`/`gcc12-arm`/`gcc13-arm` × opt × board), and two frotz 2.54 sources jobs (`gcc13-arm` × `-O2`/`-Os`) and names `postgresql://ghidra-bsim:5432/bsim` (`medium_nosize`). `docker/references.userland.yaml` is a separate 24-job x86-64 corpus pointing at the same database. `mode=framework` harvests build-tree objects, never the linked ELF. `mode=sources` accepts `prepare` (operator-supplied shell command in the cloned tree before compile; never read from the repo). Each artifact gets a `<artifact>.json` sidecar (resolved commit, compiler `--version`, sha256, `debug_path_prefix`, `prepare`); manifest skip is sidecar-hash match plus recorded `prepare`, not filename. Compile with `-g`; `strip_debug` defaults false. One `builder` service; identity selects binaries inside that image. `POST /build` returns a job id; `wait_seconds` (max 55) then `build_reference_status`. `builder_health` proxies `GET /health`. `source_read` is `POST /source` (not a job). Do not mount docker.sock into any service. |
