...
- Never open a reference program at apply time. Everything `bsim_apply_matches(apply_signatures=true)` needs is written at ingest: `<artifact>.gdt` (every DWARF type plus one function definition per DWARF-signed function under `/bsim-sig/`) and the `prototype` / `calling_convention` / `param_count` / `has_dwarf` / `gdt_path` columns of `corroboration.functions`. `BSimSignatures` owns both sides; `BSim_McpExtract.java` mirrors its export for the `ghidra://` path and must stay byte-identical in both locations.
...
- A function that already carries the proposed name is `unchanged` (`name_unchanged`), never a conflict with itself, and still runs the signature gates. `skip_named=false` plus `apply_signatures=true` is how a corpus named before this feature gets its signatures; with the self-conflict bug (measured live 2026-09-01) every such function came back `conflicting` and no signature could ever land.
