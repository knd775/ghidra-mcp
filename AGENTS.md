...
| `BSimService.java` (`BSimCli`, `BSimJobs`, `BSimUrls`, `BSimDbProbe`, `BSimMatches`, `BSimCliParser`, `BSimSignatures`, `CorroborationExtract`, `CorroborationEvidence`, `CorroborationStore`, `BSim_McpExtract.java`) | Offline: `BSimCliParserTest`, `BSimServiceValidationTest`, `CorroborationTest`, `BSimSignaturesTest`. Python: `tests/unit/test_bsim_postgres_stack.py`. Live: `tests/integration/test_bsim_cross_build.py` (skips without `GHIDRA_BSIM_FIXTURE`). Invariants below. |
| `ReferenceBuildService.java` (`ReferenceBuild`, `ReferenceManifest`, `FrameworkBuild`, `BuilderClient`, `ToolchainIdentity`), `docker/builder/*`, `docker/stubs/*` | Offline: `ReferenceBuildServiceValidationTest`. Python: `tests/unit/test_builder_stack.py`. Live: `tests/integration/test_builder_smoke.py` against a running builder (`GHIDRA_MCP_BUILDER_URL`, plus `GHIDRA_MCP_BUILDER_DATA` for the ELF checks). One call per path, ~90s, and the only tier that has ever caught a builder defect before a deploy. Invariants below. |
...
| `tools/docker_image_changes.py`, `.github/workflows/ghcr.yml` | `tests/unit/test_docker_image_changes.py` + `tests/unit/test_ci_workflow_triggers.py`. A Java-only file list must select only `headless`. |
...
- Never open a reference program at apply time. Everything `bsim_apply_matches(apply_signatures=true)` needs is written at ingest: the file `<artifact>.gdt` plus a project archive at `/refs/types/<name>-<version>.gdt` (library+version, never opt flags), and the `prototype` / `calling_convention` / `param_count` / `has_dwarf` / `gdt_path` columns of `corroboration.functions`. `type_archive_mode` is `project` (default when a project is open), `file` (`GHIDRA_MCP_TYPE_ARCHIVE_DIR`, client-resolvable), or `local` (disassociate after resolve). A missing project archive falls back to local and is counted as `types_fallback_local`. On a server-bound project, `bsim_ingest` adds the archive to version control (checkout / update / checkin on later ingests of the same key); the response reports `type_archive_versioned` and `type_archive_version`. A local project says `type_archive_versioned: false`. An unversioned project archive is invisible to every GUI client and reports nothing — worse than an error. `relink_types=true` re-associates types on functions that already carry `[bsim-sig]` with the current archive when `isEquivalent`; a missing or edited type is `relink_skipped`. `BSimSignatures` owns both sides; `BSim_McpExtract.java` mirrors its file-gdt export for the `ghidra://` path and must stay byte-identical in both locations.
- A proposed name that already exists on a *different* function is `name_exists_elsewhere`, not `conflicting`. Default: do not apply. `resolve_conflicts=best` may reassign only when every holder carries BSim provenance (`[bsim]` / `[bsim-sig]` plate or `bsim` tag) *and* a parseable `conf=`, and the candidate beats each by `conflict_min_confidence_margin`. Demotion restores `FUN_<addr>`, strips the marker and tag. A hand-named holder is never demoted. After any run, no two functions share a name.
- `rename_named` defaults to false. Keep it. The first live run suppressed 13 renames and all 13 were wrong (four application functions matched to `sleep_until_callback` by degenerate shape; five distinct Frotz opcode handlers collapsed onto `ret`). `skip_named=true`: unchanged; `rename_named` has no effect. `skip_named=false` + `rename_named=false` + `apply_signatures=true`: signatures on named functions, no name changes; disagreements are `rename_suppressed`. `rename_named=true` is the previous rename-anything behavior. Applied names stamp `[bsim] from <exe> conf=X.X` and the `bsim` function tag. The table lives in `docs/prompts/BSIM.md`.
- A wrong signature propagates through the decompiler, so signatures have their own gates and every one is counted in the response: `min_signature_confidence >= min_confidence` (default 40), same `arch` only, `has_dwarf` only, archive present, not already applied (the `[bsim-sig]` plate marker), and reference `param_count` equal to the target's decompiler-inferred count. Fail any gate and the name is still applied; the signature is not. `apply_signatures` defaults to false.
- A function that already carries the proposed name is `unchanged` (`name_unchanged`), never a conflict with itself, and still runs the signature gates. `skip_named=false` plus `apply_signatures=true` (and `rename_named=false`) is how a corpus named before this feature gets its signatures; with the self-conflict bug (measured live 2026-09-01) every such function came back `conflicting` and no signature could ever land.
- The signature source is the best hit unless it fails a reference-side gate and another hit with the same name passes (`best_hit_source` / `best_hit_reason` on the row); a hit whose marker is already on the function wins outright. Two copies of one reference tie on similarity and BSim's order between them is a coin toss (measured 2026-09-01: 24 of 40 `skipped_no_dwarf` with the DWARF copy one row down).
- `dry_run` parameter counts come from the program as it stands. A real pass types callees first and later functions can settle to a different count (`lfs_mount` mismatched 1 vs 2 in the preview, applied for real), so the preview names the gates, not the final counts. `has_dwarf` is false for externals and thunks: their `IMPORTED` signature is Ghidra's library archive, not DWARF.
- Type import uses `DataTypeConflictHandler.KEEP_HANDLER` throughout, and types the prototype names directly are matched against the target by name in any category, so a hand-defined `lfs_t` at the root beats the archive's `/DWARF/lfs.h/lfs_t`. Never replace an existing type. `dry_run` runs the gates and reports the planned type work but imports nothing.
...
- Never pass `-DCMAKE_C_FLAGS=` / `-DCMAKE_CXX_FLAGS=` to a stub. Assigning them on the command line replaces what the framework's toolchain file seeded through `CMAKE_C_FLAGS_INIT`; for pico-sdk that is `-mcpu=cortex-m0plus -mthumb`, and losing it makes every compile die in the assembler ("selected processor does not support `dmb' in ARM mode"; Cortex-M is Thumb-only). Flags go in as `GHIDRA_C_FLAGS`/`GHIDRA_CXX_FLAGS` and the stub's `CMakeLists.txt` spends them with `add_compile_options()`, which appends. Every CMake toolchain-file framework (Zephyr, ESP-IDF) has the same trap; a unit test requires every cmake stub to consume `GHIDRA_C_FLAGS`.
- Configure sets `CMAKE_BUILD_TYPE=GhidraRef` with empty per-config flag variables, so a framework that defaults to `Release` (pico-sdk does) cannot append its own `-O3 -DNDEBUG` after the requested level. The artifact name and sidecar record `opt`, and nothing downstream can check it. BSim does not know what flags an object was built with, so a mislabelled entry is worse than a failed build.
- Configure exports `compile_commands.json` and the builder verifies it before building: exactly one `-O`, matching the request (assembly sources exempt), plus the stub's `require_compile_flags` / `require_compile_flag_prefixes` (pico-sdk: `-mthumb` exactly, `-mcpu=` as a prefix, because the cpu depends on the board). A mismatch is `flag_mismatch`, an absent file is `flags_unverifiable`; success reports `flag_check` in the envelope and the sidecar. `generator: make` stubs use `CFLAGS` and have no such check.
- A harvested library the requested ones dragged in with no defined symbols (RP2040's `bs2_default_library` is assembled boot padding) is named in `failed` and skipped, not written. A *requested* library with zero functions is still a hard refuse.
...
- `sources` must stay optional at the schema level and validated per mode in the handler. When it was schema-required, the bridge's pydantic rejected every framework call before the handler ran, so a valid stub, an invalid stub, and a missing `framework` all failed identically with `outputSchema defined but no structured output returned`. Passing it with `mode=framework` is a named error, not a silent no-op.
- A framework that resolves its own toolchain does not honour `CMAKE_C_COMPILER`. pico-sdk's `pico_find_compiler` searches `PATH`, and the packed prefixes are off `PATH` by design. Stubs declare `toolchain_cache_vars` in `stub.json` (pico-sdk: `PICO_TOOLCHAIN_PATH`, `PICO_GCC_TRIPLE`), filled from the resolved identity; Zephyr (`ZEPHYR_TOOLCHAIN_VARIANT`/`CROSS_COMPILE`) and ESP-IDF (`IDF_TOOLCHAIN`) will use the same hook, so the names stay stub data. Unresolved templates are dropped, never emitted empty. The Java preview mirrors this with a `<toolchain_bin>` placeholder.
- The builder image asserts `/usr/bin/ninja` at build time and `GET /health` reports `generators`/`cmake_generator`; configure falls back to CMake's default generator when ninja is missing rather than dying on `CMAKE_MAKE_PROGRAM is not set`.
