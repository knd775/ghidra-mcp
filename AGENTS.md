...
- Never open a reference program at apply time. Everything `bsim_apply_matches(apply_signatures=true)` needs is written at ingest: the file `<artifact>.gdt` plus a project archive at `/refs/types/<name>-<version>.gdt` (library+version, never opt flags), and the `prototype` / `calling_convention` / `param_count` / `has_dwarf` / `gdt_path` columns of `corroboration.functions`. `type_archive_mode` is `project` (default when a project is open), `file` (`GHIDRA_MCP_TYPE_ARCHIVE_DIR`, client-resolvable), or `local` (disassociate after resolve). A missing project archive falls back to local and is counted as `types_fallback_local`. `BSimSignatures` owns both sides; `BSim_McpExtract.java` mirrors its file-gdt export for the `ghidra://` path and must stay byte-identical in both locations.
...
- `rename_named` defaults to false. `skip_named=true`: unchanged; `rename_named` has no effect. `skip_named=false` + `rename_named=false` + `apply_signatures=true`: signatures on named functions, no name changes; disagreements are `rename_suppressed`. `rename_named=true` is the previous rename-anything behavior. Applied names stamp `[bsim] from <exe> conf=X.X` and the `bsim` function tag.
